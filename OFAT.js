...
let mode = ''
  ...
  ; (async () => {
    ...
    try {
      ...
      const allprices = [] //an empty array
      ...
      for (let index = 1; index < lines.length; index++) {
        ...
        let Occupation = values[9]
        let Hobbies = values[10]
        let InsuranceSum = parseInt(values[11])
        let MeritRating = values[12]
        let DamageInsurance = values[13]
        let OptionalProducts = values[14]
        let CourtesyCar = values[15]
        ...
        await page
          .locator('label')
          .filter({ hasText: Hobbies.toString() })
          .locator('span')
          .click()
        await page.getByRole('button', { name: 'Next »' }).click()
        await page.getByRole('textbox', { name: 'MM/DD/YYYY' }).click()
        ...
        await page.locator('#damageinsurance').selectOption(DamageInsurance.toString())
        await page.locator('label').filter({ hasText: OptionalProducts.toString() })
        ...
        for (const option of options) {
          ...
          for (let pdfindex = 0; pdfindex < pdflines.length; pdfindex++) {
            ...
            if ((result = pdfline.match(/^\d+\/\d+\/\d+\s+(.*)+\s+\w+/))) {
              pdfseats = result[1]
            }
